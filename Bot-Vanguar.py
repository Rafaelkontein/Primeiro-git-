...
class ChromeAuto:
    ...
    def pegar_dados(self):
        pedidos = openpyxl.load_workbook('raraa.xlsx')
        ...
        for index in range(len(dados)):
            ...
            aba_ativa = pedidos.active
            for cel in aba_ativa['A']:
                if cel.value is not None:
                    linha = cel.row
                    aba_ativa[f"B{linha}"] = cart_margin
                    aba_ativa[f"B{linha}"]=margim
            pedidos.save('tabelapronta.xlsx')
